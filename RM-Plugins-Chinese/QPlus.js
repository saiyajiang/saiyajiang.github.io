...
//=============================================================================
 /*:
 * @plugindesc <QPlus> (需要放在所有的QPlugins的上方)
 * 一些MV的小改动，便于插件的开发。
 * @author Quxios  | 版本：Version 1.4.3
 *
 * @param 快速测试
 * @desc 允许进行快速测试(直接开始新游戏)
 * 用 true 或者 false 来进行设置是否允许
 * @type Boolean
 * @default true
 *
 * @param 默认启用开关
 ...
 * ============================================================================
 * 这个插件是大部分QPlugins的核心。
 * 它还为RPG Maker MV增加了一些新的功能。
 *
 * ============================================================================
 * ## 注释：Notetags / Comments
 * ============================================================================
 * **事件保留方向**
 * ----------------------------------------------------------------------------
 * 将以下内容添加到注释Notetags / Comments中
 * 将使该事件在更改页面时保留其走向
 * ~~~
 *  <retainDir>
 * ~~~
 ...
 * **地图不使用图块**
 * ----------------------------------------------------------------------------
 * 您可以通过将该注释添加到地图来禁用图块地图
 * ~~~
 *  <noTilemap>
 * ~~~
 * 这将用简单的轻量级精灵容器替换图块地图
 * 使用它可能会提高性能
 * 所以如果你的地图不使用任何图块，都是远景图，那么你应该考虑使用这个
 *
 * *请注意，如果它们调用函数，这可能会破坏一些插件原来的图块地图类型
 *
 * ============================================================================
 * ## 插件命令
 * ============================================================================
 ...
 *  wait X Y
 * ~~~
 * 如果 Y 为空, 则会在 0 - X 之间进行随机等待
 *
 * ----------------------------------------------------------------------------
 * **全局锁**
 * ----------------------------------------------------------------------------
 * 这个插件命令将'锁定'所有对象或某些对象
 * (对象：原文为 characters 包含了 事件与角色)
 * 通过锁定，你可以锁定对象移动和对象动画
 * ~~~
 ...
 *  - 1: 锁定对象移动
 *  - 2: 锁定对象移动和动画
 * - [CHARACTERS] - 可选, `Character Ids`列表去申请或忽略
 * 使用 space 分隔
 *  * CHARAID: 角色的ID号
 *   - 如果是角色 : 0, p, or player
 *   - 如果是事件 : EVENTID, eEVENTID, eventEVENTID 或者叫this的事件
 *   (用数字代替 EVENTID )
 *
 * 可能的选项:
 *  - only: 只适用于列出的角色
